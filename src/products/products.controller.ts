...
export class ProductsController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Product successfully created.', type: Product })
  ...
  async create(@Body() createProductDto: CreateProductDto, @Req() req: ExpressRequest): Promise<Product> {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved list of products.', type: [Product] })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved product details.', type: Product })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized (missing or invalid JWT)' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden (non-owner role)' })
  async findOne(@Param('id') id: string, @Req() req: ExpressRequest): Promise<Product> {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Product successfully updated.', type: Product })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input data or ingredient not found.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized (missing or invalid JWT)' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden (non-owner role)' })
  async update(@Param('id') id: string, @Body() updateProductDto: UpdateProductDto, @Req() req: ExpressRequest): Promise<Product> {
    return this.productsService.update(id, updateProductDto, (req as any).user.sub);
  }
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully calculated price change impacts.' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'One or more products not found.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input data.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized (missing or invalid JWT)' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden (non-owner role)' })
  async whatIf(@Body() whatIfDto: WhatIfDto, @Req() req: ExpressRequest): Promise<{ productId: string; newMargin: number; newStatus: string }[]> {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully calculated margin impact.' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product or ingredient not found.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid input data.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized (missing or invalid JWT)' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden (non-owner role)' })
  async milkSwap(@Body() milkSwapDto: MilkSwapDto, @Req() req: ExpressRequest): Promise<{ originalMargin: number; newMargin: number; upchargeCovered: boolean }> {
  ...
  async getMaxProducibleQuantity(@Body('productId') productId: string, @Req() req: ExpressRequest): Promise<GetMaxProducibleQuantityResponseDto> {
    return this.productsService.getMaxProducibleQuantity(productId, (req as any).user.sub);
  }
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully applied quick action.', type: Product })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Product not found.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid new sell price.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized (missing or invalid JWT)' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden (non-owner role)' })
  async quickAction(@Param('id') id: string, @Body() quickActionDto: QuickActionDto, @Req() req: ExpressRequest): Promise<Product> {
    if (quickActionDto.new_sell_price <= 0) {
      throw new BadRequestException('New sell price must be positive');
    }
    return this.productsService.quickAction(id, quickActionDto, (req as any).user.sub);
  }
}
