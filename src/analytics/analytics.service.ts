  ...
  async getDashboard(startDate: Date, endDate: Date): Promise<any> {
    ...
    try {
      ...
      const costs = sales.reduce((sum, sale) => {
        if (sale.product) {
          const cost = typeof sale.product.total_cost === 'number' ? sale.product.total_cost : (parseFloat(sale.product.total_cost) || 0);
          return sum + (cost * (typeof sale.quantity === 'number' ? sale.quantity : (parseFloat(sale.quantity) || 0)));
        }
        return sum;
      }, 0);
      ...
      const losingMoney = sales
        ...
        .reduce((acc, sale) => {
          ...
          if (existing) {
            existing.quantity += (typeof sale.quantity === 'number' ? sale.quantity : (parseFloat(sale.quantity) || 0));
            existing.margin_amount += ((typeof sale.product.margin_amount === 'number' ? sale.product.margin_amount : (parseFloat(sale.product.margin_amount) || 0)) * (typeof sale.quantity === 'number' ? sale.quantity : (parseFloat(sale.quantity) || 0)));
          } else {
            acc.push({
              name: sale.product.name,
              quantity: (typeof sale.quantity === 'number' ? sale.quantity : (parseFloat(sale.quantity) || 0)),
              margin_amount: ((typeof sale.product.margin_amount === 'number' ? sale.product.margin_amount : (parseFloat(sale.product.margin_amount) || 0)) * (typeof sale.quantity === 'number' ? sale.quantity : (parseFloat(sale.quantity) || 0))),
            });
          }
          return acc;
